...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src: torch.Tensor, src_mask: Optional[torch.Tensor] = None, src_key_padding_mask: Optional[torch.Tensor] = None, is_causal: bool = False) -> torch.Tensor:
        ...
        Args:
            ...
            src_mask: additive attention mask broadcast onto the (Q, K) logits.
                Use ``-inf`` for forbidden positions (e.g. upper-triangular for
                a causal mask). Threaded through to ``self_attn`` so
                ``nn.TransformerEncoder(..., mask=causal_mask)`` works.
        """
...
class MultiHeadAttention(nn.Module):
  ...
  def forward(self, x, attn_mask: Optional[torch.Tensor] = None):
    ...
    Args:
      ...
      attn_mask: optional additive mask of shape (Q, K) or broadcastable thereto.
          Use ``-inf`` for forbidden positions (causal masking, padding masking,
          etc.). If None, full attention is used.
    Returns:
